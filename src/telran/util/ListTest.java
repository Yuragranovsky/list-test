...
// test allocate method		
		for (int i = 0; i < 20; i++) {
			assertEquals(true, strings.add(i, "name" + i));
			assertEquals("name" + i, strings.get(i));
		}
// checking that when adding a new element to the array, the all of the element values are saved correctly
		for (int i = 0; i < 20; i++) {
			assertEquals("name" + i, strings.get(i));
		}
